...
class DBHelper private constructor(val context: Context) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    ...
    private val COL_SUN_TIMES = "sun_times"
    ...
    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL("CREATE TABLE $ALARMS_TABLE_NAME ($COL_ID INTEGER PRIMARY KEY AUTOINCREMENT, $COL_TIME_IN_MINUTES INTEGER, $COL_DAYS INTEGER, " +
            "$COL_IS_ENABLED INTEGER, $COL_VIBRATE INTEGER, $COL_SUN_TIMES INTEGER, $COL_SOUND_TITLE TEXT, $COL_SOUND_URI TEXT, $COL_LABEL TEXT)")
    ...
    private fun fillAlarmContentValues(alarm: Alarm): ContentValues {
        return ContentValues().apply {
            ...
            put(COL_SUN_TIMES, alarm.sunTimes)
        }
    }
    ...
    fun getAlarms(): ArrayList<Alarm> {
        ...
        val cols = arrayOf(COL_ID, COL_TIME_IN_MINUTES, COL_DAYS, COL_IS_ENABLED, COL_VIBRATE, COL_SOUND_TITLE, COL_SOUND_URI, COL_LABEL, COL_SUN_TIMES)
            ...
            if (cursor?.moveToFirst() == true) {
                do {
                    try {
                        ...
                        val sunTimes = cursor.getIntValue(COL_SUN_TIMES) == 1

                        val alarm = Alarm(id, timeInMinutes, days, isEnabled, vibrate, soundTitle, soundUri, label, sunTimes)
                        alarms.add(alarm)
                    } catch (e: Exception) {
                        continue
                    }
