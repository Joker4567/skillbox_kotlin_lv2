...
class EditAlarmDialog(val activity: SimpleActivity, val alarm: Alarm, val callback: (alarmId: Int) -> Unit) {
    ...
    init {
        ...
        view.apply {
            ...
            edit_sun_times_holder.setOnClickListener {
                edit_sun_times.toggle()
                alarm.sunTimes = edit_sun_times.isChecked
                if(alarm.sunTimes) {
                    val permissionGranted = activity.getPermissionLocation()
                    if (permissionGranted.not()) {
                        activity.lifecycle.coroutineScope.launchWhenStarted {
                            StatePermissionLocation.isSuccessLocation
                                .collect { isSuccess ->
                                    if (isSuccess.not()) {
                                        alarm.sunTimes = false
                                        edit_sun_times.isChecked = false
                                        activity.showErrorToast("Разрешите доступ к геолокации")
                                    } else {
                                        getLocation()
                                    }
                                }
    ...
    private fun getLocation() {
        LastLocationProvider(activity).getLocation { resultLocation ->
            resultLocation.onSuccess { location ->
                val times = SunTimes.compute()
                    .on(getCurrentDate())
                    .at(location.latitude, location.longitude)
                    .execute()
                val minutes = times.rise?.let {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
                        getDateMinutes(it.year, it.monthValue, it.dayOfMonth, it.hour, it.minute)
                    else
                        0
                } ?: 0
                alarm.timeInMinutes += minutes
            }
            resultLocation.onFailure {
                activity.showErrorToast("Ошибка получения местоположения.")
            }
        }
    }
    ...
    private fun restoreLastAlarm() {
        if (alarm.id == 0) {
            activity.config.alarmLastConfig?.let { lastConfig ->
                ...
                alarm.sunTimes = lastConfig.sunTimes
            }
        }
    }
    ...
    private fun checkDaylessAlarm() {
        ...
        view.edit_sun_times.isChecked = alarm.sunTimes
    }
